...
#define SHADER_SUB_DIR_NAME std::string("solution1")
#define SHADER_SUB_DIR_NAME_W std::wstring(L"solution1")
...
public:
	...
	{
		title = "VulkanRT Educational Solution1";
...
#ifdef _WIN32
		SpirvCompiler compiler(std::wstring(PROJECTDIR_SOLUTION1_W) + L"shaders/", getShadersPathW() + SHADER_SUB_DIR_NAME_W + L"/");
#else
		SpirvCompiler compiler(std::string(PROJECTDIR_SOLUTION1) + "shaders/", getShadersPath() + SHADER_SUB_DIR_NAME + "/");
#endif
		compiler.CompileAll();
	}
...
#pragma endregion
	...
	/*
		...
		Modified SBT Layout used in this sample:
	...
	void createRayTracingPipeline()
	{
		...
		*/
		...
		// Miss group
		{
			...
			shaderStages.push_back(loadShader(getShadersPath() + SHADER_SUB_DIR_NAME + "/shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
			shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
			shaderGroups.push_back(shaderGroup);
		}
