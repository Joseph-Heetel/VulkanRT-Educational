...
*
* Minor Modifications by Joseph Heetel
*	- Added SHADER_SUB_DIR_NAME macro for shader file directories
*	- Enabled validation layer console output
*	- Fix validation layer error by adding 'VK_EXT_debug_report' instance extension
*	- Edits to shader files for academic exercises
*	- Further edits to code to allow adding shadow ray capability
*
* License remains the same MIT license (obviously)
*/
...
#define SHADER_SUB_DIR_NAME std::string("exercise1")
#define SHADER_SUB_DIR_NAME_W std::wstring(L"exercise1")
...
public:
	...
	{
		title = "VulkanRT Educational Exercise1";
...
#ifdef _WIN32
		SpirvCompiler compiler(std::wstring(PROJECTDIR_EXERCISE1_W) + L"shaders/", getShadersPathW() + SHADER_SUB_DIR_NAME_W + L"/", true);
#else
		SpirvCompiler compiler(std::string(PROJECTDIR_EXERCISE1) + "shaders/", getShadersPath() + SHADER_SUB_DIR_NAME + "/", true);
#endif
		compiler.CompileAll();
	}
...
#pragma endregion
	...
	/*
		...
		How the SBT should look like for shadow ray casting:

			/---------------\
			| raygen		|
			|---------------|
			| miss			|
			| shadow miss	| <- this needs to be added
			|---------------|
			| hit			|
			\---------------/
	...
	*/
	void createShaderBindingTables()
	{
		...
		// TODO: Edit code of this function to add additional miss shader
		createShaderBindingTable(shaderBindingTables.raygen, 1);
		createShaderBindingTable(shaderBindingTables.miss, 1);		// Edit this line
		createShaderBindingTable(shaderBindingTables.hit, 1);

		// Copy handles
		memcpy(shaderBindingTables.raygen.mapped, shaderHandleStorage.data(), handleSize);
		memcpy(shaderBindingTables.miss.mapped, shaderHandleStorage.data() + handleSizeAligned, handleSize);	// Edit this line
		memcpy(shaderBindingTables.hit.mapped, shaderHandleStorage.data() + handleSizeAligned * 2, handleSize);		// Edit this line
	}
	...
	*/
	void createRayTracingPipeline()
	{
		...
		*/
		...
		{
			shaderStages.push_back(loadShader(getShadersPath() + SHADER_SUB_DIR_NAME + "/raygen.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR));
		...
		// Miss group
		{
			shaderStages.push_back(loadShader(getShadersPath() + SHADER_SUB_DIR_NAME + "/miss.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
			VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
			shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
			shaderGroup.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
			shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
			...
			shaderGroup.intersectionShader = VK_SHADER_UNUSED_KHR;
			shaderGroups.push_back(shaderGroup);
			// TODO: Add shaderstage and shadergroup for miss shader
			// HINT: loadShader(getShadersPath() + SHADER_SUB_DIR_NAME + "/shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR)
		}

		// Closest hit group
		{
			shaderStages.push_back(loadShader(getShadersPath() + SHADER_SUB_DIR_NAME + "/closesthit.rchit.spv", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR));
			VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
			shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
	...
	*/
	...
	void updateUniformBuffers()
	{
		...
		uniformData.lightPos = glm::vec4(cos(glm::radians(timer * 360.0f)) * 40.0f, 10.f + sin(glm::radians(timer * 360.0f)) * 20.0f, 25.0f + sin(glm::radians(timer * 360.0f)) * 5.0f, 0.0f);
